...
CCliManager::CCliManager(CNode* node) : _node(node) {

}

CCliManager::~CCliManager() {

}

void CCliManager::HandleClient(const std::string& ip_port, const std::string& msg, ::raft::ClientResponse* response,
...
bool CCliManager::SendRet(Time& time_pos, int err_code, const std::string& des) {
    std::unique_lock<std::mutex> lock(_client_mutex);
...
void CCliManager::SendNotLeader() {
